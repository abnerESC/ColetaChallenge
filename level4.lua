...
--OBJETIVO DA FASE
local objetivo1 = { "côcos", 12, 0 }
local objetivo2 = { "latas de refri", 6, 0 }
...
function scene:create( event )
	...
	local sheet_fish = graphics.newImageSheet( "img/sprits/fish10.png", sheetOptions )
	...
	local fish = display.newSprite( sheet_fish, sequences_fish )
	fish.x, fish.y = 0, 200
	fish:play()

	local iteradorFish = 0
	...
	naoObjetivo = {"copos de plástico","copos de refri","cervejas", "champagnes"}
	vectorNaoObjetivo = {}
	for i=1,26 do
	...
	local function createObjectsWithDelay()
  		...
  		if (possibleCreate == true) then
			...
			--print("Criando " .. object.name .. " de ")
	      	...
	      	if image == "cascas de banana" or image == "bananas" or image == "pizzas" or image == "côcos" then
	      		object.id = ORGANICO
	  		elseif image == "cervejas" or image == "champagnes" then
	  			object.id = VIDRO
			elseif image == "copos de café" or image == "copos de plástico" or image == "copos de refri"then
				object.id = PLASTICO
			elseif image == "talheres" or image == "compassos" or image == "latas de refri" then
				object.id = METAL
	...
	local function handleButtonEvent( event )
	    ...
	    if ( "ended" == event.phase ) then
			...
			elseif ( event.target.id == ORGANICO )then
				--print( "mudar para " .. event.target.id )
				garbage_default = display.newImageRect( "img/screenComponents/recycle_brown.png", 90, 90  )
	...
	buttonChangeColor1.id = ORGANICO
	...
	buttonChangeColor4.id = PLASTICO
